...
export default function PartOfApplicability({
  ...
  return (
    <div className="card applicability partof-applicability">
      <div className="card-head">
        <div>
          <div className="card-type">PartOf Applicability</div>
          <div className="card-sub">Must be related by IFCRELAGGREGATES to an Entity. The entity must ??.</div>
        </div>
      ...
      <div className="card-body">
        <div className="row">
          <label>Relation <span className="optional">optional</span></label>
          <div style={{ display: 'flex', gap: 8, width: '100%' }}>
            <select value={item.relation || ''} onChange={(e) => update({ relation: e.target.value })} style={{ flex: 1 }}>
